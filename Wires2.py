...
def setOrCreate(GRID, X,Y, val):
    pass


def right(GRID, steps, origin, inc):
    for i in range(1, steps+1):
        GRID[origin[0]+i][origin[1]] += inc
    return (origin[0]+i, origin[1])

def left(GRID, steps, origin, inc):
    for i in range(1, steps+1):
        GRID[origin[0]-i][origin[1]] += inc
    return (origin[0]-i, origin[1])

def up(GRID, steps, origin, inc):
    for i in range(1, steps+1):
        GRID[origin[0]][origin[1]+i] += inc
    return (origin[0], origin[1]+i)

def down(GRID, steps, origin, inc):
    for i in range(1, steps+1):
        GRID[origin[0]][origin[1]-i] += inc
    return (origin[0], origin[1]-i)

def move(GRID, action, origin, inc):
    direction = action[0]
    steps = int(action[1:])
    O = origin

    if direction == "R":
        O = right(GRID, steps, origin, inc)
    elif direction == "L":
        O = left(GRID, steps, origin, inc)
    elif direction == "D":
        O = down(GRID, steps, origin, inc)
    elif direction == "U":
        O = up(GRID, steps, origin, inc)
    return O
...
if __name__ == "__main__":
    ...
    GRID = np.zeros((11000, 11000), dtype=np.uint8)



    for P in L1:
        inc = 1
        origin = move(GRID, P, origin, inc)
