...
W1 = Wire1.split(",")
W2 = Wire2.split(",")
TEST0a = "R8,U5,L5,D3".split(",")
TEST0b = "U7,R6,D4,L4".split(",")
...
def right(steps, cursor, GRID):
    cx = cursor[0]
    cy = cursor[1]
    for x in range(steps+1):
        GRID[cy][cx+x] = GRID[cy][cx+x] + cross(GRID[cy][cx+x], x)
    return(cx+x, cy)


def left(steps, cursor, GRID):
    cx = cursor[0]
    cy = cursor[1]
    for x in range(steps+1):
        if (cx-x) < 0:
...
def up(steps, cursor, GRID):
    cx = cursor[0]
    cy = cursor[1]
    for x in range(steps+1):
        if (cy-x) < 0:
            raise Exception("Negative index")
        GRID[cy-x][cx] = GRID[cy-x][cx] + cross(GRID[cy-x][cx], x)
    return(cx, cy-x)


def down(steps, cursor, GRID):
    cx = cursor[0]
    cy = cursor[1]
    for x in range(steps+1):
        GRID[cy+x][cx] = GRID[cy+x][cx] + cross(GRID[cy+x][cx], x)
...
def drawGrid(path, START, GRID):
    ...
    origin = START
    for p in path:
        print(p[0], int(p[1:]), origin)
        origin = symbols[p[0]](int(p[1:]), origin, GRID)


def withMatplot():
    START = (5000, 20999)
    GRID = np.zeros((21000, 21000), dtype=np.uint8)
    # GRID.astype(int)
    drawGrid(W1, START, GRID)
    drawGrid(W2, START, GRID)
    #fig = plt.figure()
    #ax1 = fig.add_subplot(121)
    # Bilinear interpolation - this will look blurry
    #ax1.imshow(GRID, interpolation='bilinear', cmap=cm.Greys_r)
    plt.matshow(GRID, cmap='binary')
    plt.colorbar()
    plt.show()


def draw(path, origin):
    R = [origin]
    for p in path:
        x = R[-1][0]
        y = R[-1][1]
